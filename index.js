...
const renderCalendar = () => {
    ...
    // Reset selectedDay when rendering a new month
    selectedDay = null; 
    ...
    for (let day = 1; day <= daysInMonth; day++) {
        ...
        dayElement.addEventListener('click', () => {
            ...
            srchinp3.value = `${selectedDay} ${currentDate.toLocaleString('default', { month: 'short' })} ${year}`;
...
const enupdatePlaceholder = () => {
    ...
    srchinp5.value = count > 0 ? `${count}` : '0'; 
      srchinp4.value = count === 0 ? '0 Person' : `${count} Persons`; 
...
srchinp5.value = count > 0 ? `${count}` : '0'; 
srchinp4.value = count === 0 ? '0 நபர்' : `${count} நபர்கள்`; 
}
